...
static unsigned int global_seed_counter = 1; // Semilla inicial (se puede cambiar con función setSeed)
...
tSolution<int> ParProblem::createSolution()
{
    ...
    // cada numero el número del clúster
    ...
    // (En greedy por ejemplo los borramos porque los indices iniciales nunca se cambian pero el resto sí)
...
// Esta función en esta práctica no se usa
...
bool ParProblem::loadData(const string &dataPath, const string &constPath)
{
    ...
    if (!dataFile.is_open())
    {
        ...
        return false; // Para darr error en el main
    }
...
int ParProblem::countInstanceViolations(int instanceId, int clusterId, const tSolution<int> &solution)
{
    ...
    // Recorremos solo las restricciones de esta instancia específica
    for (const auto &constraint : instanceConstraints[instanceId])
    {
        ...
        if (solution[otherId] != 0)
        {
            ...
            { // Must-Link
                // Si tienen que estar juntos pero el otro está en un cluster distinto
                if (clusterId != solution[otherId]) violations++;
            }
            else
            { // Cannot-Link
...
// Dados una instancia y un centroide, calcular su distancia
...
// Actualizar centroides de una solución
// Eg si Sol = [1,2,3,1] y elem en 1D son: [1,2,3,4]
// centroids sería centroids[0] = (1+4)/2 = 2.5, centroids[1] = 2, centroids[2] = 3
...
double ParProblem::calcular_nuevo_menos_actual(const tSolution<int> &sol, int pos, int nuevo_c, const vector<int> &num_elem)
{
    ...
    for (int i = 0; i < size; ++i)
    {
        if (sol[i] == antiguo_c)
        {
            for (int d = 0; d < dims; ++d)
                mu_a_antes[d] += data[i][d];
        }
        else if (sol[i] == nuevo_c)
        {
            for (int d = 0; d < dims; ++d)
                mu_b_antes[d] += data[i][d];
        }
    }
    ...
    for (int i = 0; i < size; ++i)
    {
        if (sol[i] == antiguo_c)
        {
            ...
            if (i != pos)
                desv_b_antes += distanceToExplicitCentroid(i, mu_a_despues);
        }
        else if (sol[i] == nuevo_c)
        {
...
void ParProblem::calculateLambda()
{
    ...
    for (size_t i = 0; i < size; ++i)
    {
        ...
        { // el vector es simétrico, dist(i,j) = dist(j,i)
            double dist = 0;
...
// Calcula el centroide de un conjunto de índices (nos dan a q clúster pertenece cada elem)
...
// Calcular la desviación de cada clúster
...
// Calcular las violaciones de una solución
